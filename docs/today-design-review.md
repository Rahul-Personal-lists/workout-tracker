...
1. **Remove `userScalable: false` and `maximumScale: 1`** in [src/app/layout.tsx](../src/app/layout.tsx). One-line WCAG critical fix. Affects every screen.
2. **Add `:focus-visible` styling** to `btn-primary`, `btn-secondary`, `btn-ghost-add` in [globals.css](../src/app/globals.css), and to bottom-nav `<Link>`. Use accent ring with offset against the dark background.
3. **Bump exercise progression hint from `text-neutral-500` to `text-neutral-400`** in [today/page.tsx:190](../src/app/(app)/today/page.tsx). Also matches the sets/reps meta on the same card.
4. **Add `aria-current="page"` and `aria-label="Primary"`** to BottomNav.
5. **Demote greeting from `<h2>` to `<p>`** (or upgrade workout title to `<h1>` and demote greeting to `<h2>` — pick one and apply consistently). Right now h2 precedes h1.

These five fixes are roughly 15–20 minutes of work and resolve every Critical and Major finding above.

## 8. Nice-to-haves (when you have a quiet hour)

6. Consolidate Deload signaling — one banner instead of per-exercise hint repetition.
7. Bump empty-state CTA from `h-11` to `h-12`.
8. Add a card utility (`@utility card`) so the four card occurrences share one source of truth.
9. Wrap 🎉 in `aria-hidden` or swap for a Lucide icon.
10. Introduce `--color-text-muted` / `--color-text-subtle` tokens to centralize neutral-400/500 contrast decisions.
